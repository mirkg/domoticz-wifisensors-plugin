...
class Device():
    ...
    def get_device(self, address):
        ...
        return configuration.get_wifisensors_feature_device(address, feature_name, endpoint)
    ...
    def _create_device(self, device_data):
        device_address = device_data['ieee_address']
        feature_name = self._get_feature_name()
        endpoint = self._get_endpoint()
        alias_config = configuration.get_alias_by_wifisensors(device_address, feature_name, endpoint)
        ...
        if self.device_name_suffix != '':
            device_name = device_data['name'] + self.device_name_suffix
        elif hasattr(self, 'feature'):
            device_name = device_data['name'] + ' (' + self.feature['property'] + ')'
        else:
        ...
        configuration.set_wifisensors_feature_device(device_address, feature_name, endpoint, device_id, unit, self.alias)

        return device

    def create_device(self, unit, device_id, device_name):
        domoticz.error(
            'Unable to create device to handle "' + self.value_key + '" value for device "' + device_name + '"'
        )
    ...
    def register(self, device_data):
        ...
        if device != None:
            ...
            endpoint = self._get_endpoint()
            configuration.set_wifisensors_feature_device(device_address, feature_name, endpoint, device_id, device.Unit, self.alias)
            return device
        else:
    ...
    def get_device_args(self, value, device):
        return {
    ...
    def handle_message(self, device_data, message):
        ...
        try:
            device_values = dict(message)
        except:
            domoticz.error('Can\'t calculate the value for device ' + str(device.ID) + ' from raw value "' + str(message) + '"')
            device_values = dict({})
